...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  #Compute UdotVc and send it through softmax to get y_hat
  UdotVc = outside_vectors.dot(center_word_vec.transpose())
  y_hat = softmax(UdotVc)
  y_hat_loss = -np.log(y_hat)
  loss = y_hat_loss[outside_word_idx]

  y = np.zeros(y_hat.shape)
  y[outside_word_idx] = 1

  grad_center_vec = outside_vectors.transpose().dot(y_hat - y)
  y_hat_minus_y = y_hat-y
  grad_outside_vecs = y_hat_minus_y.reshape(len(y_hat_minus_y), 1).dot(center_word_vec.reshape(1, len(center_word_vec)))
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for outside_word in outside_words:
    loss_current, gradc, grado = word2vec_loss_and_gradient(center_word_vectors[word2ind[current_center_word]], word2ind[outside_word], outside_vectors, dataset)
