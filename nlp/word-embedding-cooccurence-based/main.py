...
def distinct_words(corpus):
    ...
    corpus_words = dict.keys()
    corpus_words = sorted(corpus_words)
    num_corpus_words = len(corpus_words)
    #print(f'corpus_words:{corpus_words}, num:{num_corpus_words}')
...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    # ### START CODE HERE ###
    M = np.mat(np.zeros([num_words, num_words]))
    word2Ind = {k: v for v, k in enumerate(words)}
    for doc in corpus:
        #print(f'len of doc: {len(doc)}')
        #print(f'doc: {doc}')
        for center in range(0, len(doc)):
          # iterate from center word to window on either side
          center_word = doc[center]
          for i in range(max(center-window_size,0), min(center+window_size, len(doc)-1)+1):
              if i==center:
                  continue
              M[word2Ind[doc[center]], word2Ind[doc[i]]] += 1
              #M[word2Ind[doc[i]], word2Ind[doc[center]]] += 1
    # ### END CODE HERE ###
...
def reduce_to_k_dim(M, k=2):
    ...
    M_reduced = svd.fit_transform(np.asarray(M))
    # ### END CODE HERE ###
